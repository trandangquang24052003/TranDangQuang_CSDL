...
class LinkedList:
    ...
    def removeany(self, position):  # Tạo hàm xóa tại vị trí position
        ...
        while i < position - 1:  # Vòng lặp chạy kiểm tra đến vị trí trước giá trị cần xóa
            p = p._next  # Trỏ đến phần tử kế tiếp
            i = i + 1  # Tăng giá trị của i

        e = p._next._element  # Biến e gán bằng giá trị của phần tử cần xóa
        p._next = p._next._next  # Trỏ giá trị tới nút hiện tại không trỏ tới giá trị cần xóa nữa mà trỏ tới phần tử tiếp theo của phần tử cần xóa
...
elelast = L.removelast()  # Gọi phương thức removelast() để xóa phần tử cuối cùng và lưu giá trị vào biến elelast
L.display()  # Hiển thị danh sách liên kết sau khi xóa phần tử cuối cùng
print("element remove ", elelast)  # In ra giá trị của phần tử đã bị xóa từ cuối danh sách liên kết
eleany = L.removeany(3)  # Gọi phương thức removeany để xóa phần tử ở vị trí 3 và lưu giá trị vào biến 'eleany'
L.display()  # Hiển thị danh sách liên kết sau khi xóa phần tử
print("element remove ", eleany)  # In ra giá trị của phần tử đã bị xóa từ danh sách liên kết
